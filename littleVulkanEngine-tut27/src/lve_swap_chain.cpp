...
VkResult LveSwapChain::submitCommandBuffers(
  ...
  */
  ...
  // Do NOT change an internal currentFrame here — renderer owns frame index now
...
void LveSwapChain::createSyncObjects() {
  ...
  renderFinishedSemaphores.resize(imageCount());  // <-- use per-frame


  std::cout << "render Finished Semaphores = " << renderFinishedSemaphores.size() << std::endl;
